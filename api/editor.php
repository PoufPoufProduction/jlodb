...
if (!$error) {
	if (array_key_exists("action",$_GET)) {
		if ( $_GET["action"]=="new") {
			...
			if (array_key_exists("description",$_POST)) { $description=$_POST["description"]; }
			if (array_key_exists("data",$_POST)) 		{ $data=$_POST["data"]; }
			if (array_key_exists("activity",$_POST)) 	{ $activity=$_POST["activity"]; }
			
			mysqli_query($link, "INSERT INTO `".$_SESSION['prefix']."editor` (`Editor_Email`,`Editor_Activity`, `Editor_Description`, `Editor_Parameters`, `Editor_State`, `Editor_Date`) VALUES ('".
